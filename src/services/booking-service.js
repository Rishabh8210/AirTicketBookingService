    ...
    async cancelFlight(bookingId){
        try {
            const response = await db.sequelize.transaction(async (t) => {
                ...
                const bookedTime = bookingDetails.dataValues.createdAt;
                // console.log(bookingDetails.dataValues.flightId);
                const flightDetailsRequestUrl = `${FLIGHT_SERVICE_PATH}/api/v1/flights/${bookingDetails.dataValues.flightId}`;
                ...
                if(isCancellable){
                    ...
                    const updateFlightDetailsRequestUrl = `${FLIGHT_SERVICE_PATH}/api/v1/flights/${bookingDetails.dataValues.flightId}`;
                    const finalResponse = await axios.patch(updateFlightDetailsRequestUrl, {totalSeats: flightDetails.totalSeats + bookingDetails.dataValues.noOfSeats});
